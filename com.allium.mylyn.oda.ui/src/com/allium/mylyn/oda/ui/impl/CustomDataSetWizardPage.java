...
public class CustomDataSetWizardPage extends DataSetWizardPage
{
    ...
    private Control createPageControl( Composite parent )
    {
        ...
        final Set<AbstractTaskContainer> elements = getTaskContainers();
        
//        contentProvider = new TaskRepositoriesContentProvider();
        contentProvider = new ArrayContentProvider();
		viewer = new TreeViewer(composite, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.SINGLE);
		data = GridDataFactory.fillDefaults().create();
		viewer.getTree().setLayoutData( data );
		
		viewer.setContentProvider(new TaskRepositoriesContentProvider() {
			@Override
...
//		viewer.setFilters(filters);
		viewer.setLabelProvider(new DecoratingLabelProvider(new TaskElementLabelProvider(),
...
//		viewer.setSorter(new TaskRepositoriesViewSorter());
	...
	private Set<AbstractTaskContainer> getTaskContainers() {
		TaskList taskList = TasksUiPlugin.getTaskList();
        final Set<AbstractTaskContainer> elements = taskList.getRootElements();
		return elements;
	}
	...
	private void initializeControl( )
	{
        ...
        for (AbstractTaskContainer container : getTaskContainers()) {
			if (queryText.equals(container.getUrl())) {
...
//            getQueryText() != null && getQueryText().trim().length() > 0 );
		boolean isValid = viewer.getSelection().isEmpty() ? false : getSelectedQuery() != null;
	...
	private IRepositoryQuery getSelectedQuery() {
		Object first = ((IStructuredSelection) viewer.getSelection()).getFirstElement();
		if (first instanceof IRepositoryQuery)
			return (IRepositoryQuery) first;
		return null;
	}
...
//        dataSetDesign.setQueryText( queryText );
		dataSetDesign.setQueryText(getSelectedQuery().getUrl());
        ...
        try
        {
             ...
             */
            IDriver customDriver = new com.allium.mylyn.oda.impl.Driver();
    ...
    private void updateDesign( DataSetDesign dataSetDesign,
                               IConnection conn, IRepositoryQuery iRepositoryQuery )
        throws OdaException
    {
        IQuery query = conn.newQuery( null );
        query.prepare( iRepositoryQuery.getUrl() );
